...
import static zms.demo.colorprogress.GoogleProgressBarUtils.checkColors;
import static zms.demo.colorprogress.GoogleProgressBarUtils.checkNotNull;
import static zms.demo.colorprogress.GoogleProgressBarUtils.checkPositive;
import static zms.demo.colorprogress.GoogleProgressBarUtils.checkPositiveOrZero;
import static zms.demo.colorprogress.GoogleProgressBarUtils.checkSpeed;

public class GoogleProgressDrawable extends Drawable implements Animatable {
	...
	private GoogleProgressDrawable(Interpolator interpolator,
	...
	public static class Builder {
		...
		public GoogleProgressDrawable build() {
			if (mGenerateBackgroundUsingColors) {
				mBackgroundDrawableWhenHidden = GoogleProgressBarUtils
						.generateDrawableWithColors(mColors, mStrokeWidth);
			}
			GoogleProgressDrawable ret = new GoogleProgressDrawable(
		...
		private void initValues(Context context, boolean editMode) {
			...
			if (!editMode) {
				mSectionsCount = res
						.getInteger(R.integer.default_sections_count);
				mSpeed = Float
						.parseFloat(res.getString(R.string.default_speed));
				mReversed = res.getBoolean(R.bool.default_reversed);
				mProgressiveStartActivated = res
						.getBoolean(R.bool.default_progressiveStart_activated);
				mColors = new int[] { res.getColor(R.color.default_color) };
				mStrokeSeparatorLength = res
						.getDimensionPixelSize(R.dimen.default_stroke_separator_length);
				mStrokeWidth = res
						.getDimensionPixelOffset(R.dimen.default_stroke_width);
			} else {
				mSectionsCount = 4;
				mSpeed = 1f;
				mReversed = false;
